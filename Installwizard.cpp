...
void Installwizard::prepareDrive(const QString &drive) {
    ...
    InstallerWorker *installWorker = new InstallerWorker;
    installWorker->setDrive(drive);

    QThread *thread = new QThread;
    installWorker->moveToThread(thread);

    connect(thread, &QThread::started, installWorker, &InstallerWorker::run);
    connect(installWorker, &InstallerWorker::logMessage, this, [this](const QString &msg) { appendLog(msg); });
    connect(installWorker, &InstallerWorker::errorOccurred, this, [this](const QString &msg) {
        QMessageBox::critical(this, "Error", msg);
    });
    connect(installWorker, &InstallerWorker::installComplete, thread, &QThread::quit);
    connect(installWorker, &InstallerWorker::installComplete, installWorker, &QObject::deleteLater);
...
void Installwizard::createDefaultPartitions(const QString &drive) {
    ...
    PartitionWorker *partWorker = new PartitionWorker;
    partWorker->setDrive(drive);

    QThread *thread = new QThread;
    partWorker->moveToThread(thread);

    connect(thread, &QThread::started, partWorker, &PartitionWorker::run);
    connect(partWorker, &PartitionWorker::logMessage, this, [this](const QString &m) { appendLog(m); });
    connect(partWorker, &PartitionWorker::errorOccurred, this, [this](const QString &m) {
        QMessageBox::critical(this, "Error", m);
    });
    connect(partWorker, &PartitionWorker::finished, this, [this](const QString &drv) {
        populatePartitionTable(drv);
    });
    connect(partWorker, &PartitionWorker::finished, thread, &QThread::quit);
    connect(partWorker, &PartitionWorker::finished, partWorker, &QObject::deleteLater);
    connect(thread, &QThread::finished, thread, &QObject::deleteLater);
...
void Installwizard::on_installButton_clicked() {
    ...
    systemWorker->setParameters(selectedDrive, username, password, rootPassword, desktopEnv);

    QThread *thread = new QThread;
    systemWorker->moveToThread(thread);

    connect(thread, &QThread::started, systemWorker, &SystemWorker::run);
    connect(systemWorker, &SystemWorker::logMessage, this, [this](const QString &msg) { appendLog(msg); });
    connect(systemWorker, &SystemWorker::errorOccurred, this, [this](const QString &msg) {
        QMessageBox::critical(this, "Error", msg);
    });
    connect(systemWorker, &SystemWorker::finished, thread, &QThread::quit);
    connect(systemWorker, &SystemWorker::finished, systemWorker, &QObject::deleteLater);
    connect(thread, &QThread::finished, thread, &QObject::deleteLater);
